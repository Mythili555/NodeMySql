 ...
 * @returns {string}
 */
 const generateToken = (id, expires, type, secret = 'a18ZzdHWRx') => {
  const payload = {
    sub: id,
    iat: moment().unix(),
    exp: expires.unix(),
...
const generateAuthTokens = async (user,deviceType,deviceId) => {
  ...
  const refreshTokenExpires = moment().add(35, 'days');
...
const decodeToken = async (token) => {
  ...
  console.log(AccessToken,"lll");
  try {
    const decodedToken = jwt.verify(AccessToken, 'a18ZzdHWRx')
    return decodedToken.sub; // "sub" is the key used for user ID in the token
  } catch (err) {
